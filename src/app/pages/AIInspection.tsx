...
import { Upload, Camera, X, Check, Sparkles, Car as CarIcon, Bike } from "lucide-react";
...
export function AIInspection() {
  ...
  const handleFileUpload = (position: string, file: File) => {
    setUploads((prev) => ({ ...prev, [position]: file }));
  };

  const removeUpload = (position: string) => {
    setUploads((prev) => {
